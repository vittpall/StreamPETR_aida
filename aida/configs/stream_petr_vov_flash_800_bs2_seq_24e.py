...
data_root = './data/Dataset_FineTuning/'
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root,
        #ann_file=data_root + 'fine_tuning_train.pkl',
        ann_file=data_root + 'fine_tuning_train_streampetr_carlo.pkl',
        num_frame_losses=num_frame_losses,
        seq_split_num=1, # streaming video training 1
        seq_mode=True, # streaming video training FALSE
        ...
        box_type_3d='LiDAR'),
    val=dict(type=dataset_type, pipeline=test_pipeline, collect_keys=collect_keys + ['img', 'img_metas'], queue_length=queue_length, ann_file=data_root + 'val_fine_tuning_streampetr.pkl', classes=class_names, modality=input_modality),
...
optimizer = dict(
    type='AdamW', 
    lr=1e-4, # bs 8: 2e-4 || bs 16: 4e-4 || bs 2: 1e-4
    paramwise_cfg=dict(
        custom_keys={
            'img_backbone': dict(lr_mult=0),  # Freeze completely
            'img_neck': dict(lr_mult=0),
            'img_roi_head': dict(lr_mult=0),
            'pts_bbox_head.transformer.decoder': dict(lr_mult=0.1),  # Lower LR
        }),
    weight_decay=0.01)


#TODO added from SparseBEV/configs/r50_nuimg_704x256.py
log_config = dict(
    interval=1,
    hooks=[
        dict(type='TextLoggerHook', interval=1, reset_flag=True),
        #dict(type='TensorboardLoggerHook', interval=500, reset_flag=True)
    ]
)
...
load_from='work_dirs/stream_petr_vov_flash_800_bs2_seq_24e/latest.pth'
resume_from=None